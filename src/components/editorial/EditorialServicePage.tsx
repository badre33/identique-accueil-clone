    ...
    <main className="link-editorial">
      ...
      <section className="editorial-section bg-[#d8cec1]">
        <div className="editorial-shell">
          ...
          <div className="mt-16 grid border-l border-t border-black/20 sm:grid-cols-2 lg:grid-cols-4">
            {pillars.map((item, index) => <article key={item.title} className="editorial-index-card flex flex-col justify-between p-7"><span className="font-mono text-xs text-black/35">{String(index + 1).padStart(2, "0")}</span><div><h3 className="text-xl font-medium tracking-[-.035em] sm:text-2xl">{item.title}</h3><p className="mt-5 text-sm leading-7 text-black/60">{item.description}</p></div></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="editorial-section bg-[#bfd0c8]">
        <div className="editorial-shell grid gap-14 lg:grid-cols-[.85fr_1.15fr]">
          <div><p className="editorial-eyebrow text-black/45">Situations</p><h2 className="editorial-title mt-7">{useCasesTitle}</h2></div>
          <div className="border-t border-black/20">{useCases.map((item, index) => <div key={item} className="grid grid-cols-[42px_1fr] items-start border-b border-black/20 py-5 text-sm leading-6"><span className="font-mono text-[10px] text-black/35">{String(index + 1).padStart(2, "0")}</span><span className="flex gap-3"><Check className="mt-1 h-4 w-4 shrink-0" />{item}</span></div>)}</div>
        </div>
      </section>
