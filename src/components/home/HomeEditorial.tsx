  ...
  <main className="link-editorial bg-[#0a0a0a] text-white">
    <section className="link-cover relative min-h-[calc(100svh-4rem)] overflow-hidden bg-[#f4f1eb] text-[#0a0a0a] sm:min-h-[calc(100svh-5rem)]">
      ...
      <div className="link-cover__canvas mx-auto grid min-h-[calc(100svh-4rem)] max-w-[1600px] grid-rows-[auto_1fr_auto] px-5 pb-6 pt-8 sm:min-h-[calc(100svh-5rem)] sm:px-8 sm:pb-8 lg:px-12 lg:pb-10">
        <div className="motion-intro motion-intro--1 flex items-center justify-between border-b border-black/20 pb-4 text-[10px] font-semibold uppercase tracking-[0.2em] sm:text-[11px]">
        ...
        <div className="grid items-center gap-8 py-10 lg:grid-cols-[minmax(0,1fr)_minmax(300px,390px)] lg:gap-14 lg:py-12">
          <div className="relative z-10">
            <p className="motion-intro motion-intro--1 mb-6 flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[0.23em]"><span className="h-2.5 w-2.5 rounded-full bg-[#d7e942]" />Pôle marque & marketing externalisé</p>
            <h1 className="link-cover__title motion-intro motion-intro--2 max-w-[1050px] text-[clamp(3.25rem,8.4vw,9.5rem)] font-semibold uppercase leading-[0.79] tracking-[-0.075em]">Le cap.<br />L’image.<br /><span className="link-cover__outline">L’impact.</span></h1>
            <div className="motion-intro motion-intro--3 mt-9 flex max-w-3xl flex-col gap-6 border-t border-black/20 pt-5 md:flex-row md:items-start md:justify-between">
          ...
          <aside className="link-cover__dossier motion-intro-clip relative mx-auto w-full max-w-[390px] lg:mx-0" aria-label="Présentation de Link Agency">
        ...
        <div className="motion-intro motion-intro--4 flex flex-col gap-3 border-t border-black/20 pt-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <a href="/documents/LinkAgency_Dossier_Marque_2026_FINAL.pdf" download="LinkAgency_Presentation_2026.pdf" data-analytics-location="home_hero_presentation" className="group inline-flex min-h-12 items-center justify-center gap-3 border border-black bg-[#765fc4] px-5 text-[10px] font-semibold uppercase tracking-[0.16em] text-white transition hover:-translate-y-0.5 hover:bg-black" aria-label="Télécharger la présentation commerciale Link Agency 2026 au format PDF">Télécharger la présentation <span className="hidden text-white/65 sm:inline">PDF · 14 pages</span><FileDown className="h-4 w-4" /></a>
        </div>
      </div>
    </section>
    ...
    <section className="bg-[#f4f1eb] px-5 py-24 text-[#0a0a0a] sm:px-8 sm:py-32 lg:px-12 lg:py-40">
      <div className="mx-auto max-w-[1440px]">
        <div className="grid gap-8 border-b border-black pb-10 lg:grid-cols-[220px_1fr]" data-reveal><p className="editorial-eyebrow">01 / Le diagnostic</p><h2 className="max-w-5xl text-[clamp(2.6rem,5.6vw,6rem)] font-medium leading-[.93] tracking-[-.06em]">Plus d’actions ne créent pas plus de direction.</h2></div>
        <div className="mt-8 grid gap-4 lg:grid-cols-3">{situations.map((item, index) => <article key={item.n} data-reveal data-reveal-delay={index * 85} className={`${item.color} group flex min-h-[360px] flex-col justify-between border border-black p-7 transition-transform duration-500 hover:-translate-y-2 sm:p-9`}><div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[.18em]"><span>Situation {item.n}</span><span>→</span></div><div><h3 className="text-2xl font-medium leading-tight tracking-[-.035em] sm:text-3xl">{item.title}</h3><p className="mt-7 border-t border-black/30 pt-5 text-sm leading-6 text-black/65">{item.text}</p></div></article>)}</div>
      </div>
    </section>
    ...
    <section className="relative overflow-hidden border-y border-white/20 bg-[#765fc4] px-5 py-24 sm:px-8 sm:py-32 lg:px-12 lg:py-40">
      <div className="relative mx-auto grid max-w-[1440px] gap-16 lg:grid-cols-[minmax(300px,.72fr)_1.28fr] lg:gap-24">
        <div className="lg:sticky lg:top-32 lg:self-start" data-reveal="left">
          ...
          <p className="mt-10 max-w-sm border-t border-white/35 pt-6 text-sm leading-7 text-white">Un même regard tient le cap, organise les expertises et juge les résultats. Votre marketing cesse d’être une addition de prestataires.</p>
        </div>
        <div className="border-t border-white/25">
          {[{n:"01",tag:"Décider",t:"Le cap avant les canaux",d:"Nous clarifions les priorités, les arbitrages et la feuille de route avant de lancer la production.",color:"group-hover:bg-[#0a0a0a]"},{n:"02",tag:"Organiser",t:"Les bonnes compétences pour la mission",d:"Marque, contenu, acquisition et partenaires travaillent dans le même système, sous une direction identifiable.",color:"group-hover:bg-[#c86b4a] group-hover:text-black"},{n:"03",tag:"Améliorer",t:"La mesure au service des décisions",d:"Les résultats servent à choisir, corriger et concentrer l’investissement — pas à remplir un reporting.",color:"group-hover:bg-[#bfd0c8] group-hover:text-black"}].map((item, index) => <article key={item.n} data-reveal data-reveal-delay={index * 65} className={`group grid min-h-[235px] gap-8 border-b border-white/30 p-6 transition-colors duration-500 sm:p-8 md:grid-cols-[64px_1fr] ${item.color}`}><span className="text-xs opacity-75">{item.n}</span><div className="flex flex-col justify-between"><p className="text-[10px] font-bold uppercase tracking-[.2em] opacity-85">{item.tag}</p><div className="mt-14"><h3 className="text-3xl font-medium leading-tight tracking-[-.04em] sm:text-4xl">{item.t}</h3><p className="mt-5 max-w-xl text-sm leading-7">{item.d}</p></div></div></article>)}
        </div>
      </div>
    </section>
    ...
    <section className="bg-[#d8cec1] px-5 py-24 text-[#0a0a0a] sm:px-8 sm:py-32 lg:px-12 lg:py-40">
      <div className="mx-auto max-w-[1440px]">
        <div className="flex flex-col justify-between gap-7 border-b border-black pb-9 lg:flex-row lg:items-end" data-reveal><div><p className="editorial-eyebrow">03 / Dossiers sélectionnés</p><h2 className="mt-7 text-[clamp(2.8rem,5vw,5.5rem)] font-medium leading-[.93] tracking-[-.055em]">La preuve par le travail.</h2></div><Link to="/collaborations" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.16em]">Voir les missions <ArrowUpRight className="h-4 w-4" /></Link></div>
        <div className="mt-8 grid gap-4 md:grid-cols-2">{dossiers.map((item, index) => <Link to="/collaborations" key={item.client} data-reveal data-reveal-delay={(index % 2) * 85} className={`${item.color} group flex min-h-[330px] flex-col justify-between border border-black p-7 transition-transform duration-500 hover:-translate-y-1 sm:p-9`}><div className="flex justify-between text-[10px] font-bold uppercase tracking-[.18em]"><span>Dossier 0{index + 1}</span><ArrowUpRight className="h-4 w-4 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1" /></div><div><p className="text-[10px] font-bold uppercase tracking-[.18em] opacity-55">{item.label}</p><h3 className="mt-3 text-4xl font-medium tracking-[-.045em] sm:text-5xl">{item.client}</h3><p className="mt-6 max-w-xl border-t border-current/30 pt-5 text-sm leading-6 opacity-70">{item.mission}</p></div></Link>)}</div>
      </div>
    </section>

    <section id="expertises" className="bg-[#f4f1eb] px-5 py-24 text-[#0a0a0a] sm:px-8 sm:py-32 lg:px-12 lg:py-40">
      <div className="mx-auto max-w-[1440px]">
        <div className="grid gap-8 lg:grid-cols-[220px_1fr]" data-reveal><p className="editorial-eyebrow">04 / Le système</p><h2 className="max-w-5xl text-[clamp(2.8rem,5.4vw,5.8rem)] font-medium leading-[.94] tracking-[-.06em]">Le branding est le cœur.<br />Le marketing le met en mouvement.</h2></div>
        <div className="mt-16 border-t border-black">{expertises.map((item, index) => <Link key={item.to} to={item.to} data-reveal data-reveal-delay={index * 55} className={`group grid gap-5 border-b border-black py-7 transition-all duration-500 sm:px-5 md:grid-cols-[80px_1fr_1fr_30px] md:items-center ${item.tone}`}><span className="text-xs opacity-45">{item.n}</span><h3 className="text-xl font-medium tracking-tight sm:text-2xl">{item.title}</h3><p className="max-w-xl text-sm leading-6 opacity-60">{item.text}</p><ArrowUpRight className="hidden h-5 w-5 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1 md:block" /></Link>)}</div>
      </div>
    </section>

    <section className="grid bg-[#0a0a0a] lg:grid-cols-2">
      <div className="min-h-[520px] bg-[#765fc4] p-8 text-white sm:p-12 lg:p-16"><div className="flex h-full flex-col justify-between" data-reveal="left"><p className="editorial-eyebrow text-white/65">05 / Dans l’ombre</p><blockquote className="max-w-2xl text-[clamp(2.4rem,4.7vw,5.2rem)] font-medium leading-[.94] tracking-[-.055em]">« La marque doit être visible. Pas nécessairement celui qui la dirige. »</blockquote><p className="border-t border-white/40 pt-5 text-xs font-semibold uppercase tracking-[.16em]">Badre Harkaoui · Fondateur & direction stratégique</p></div></div>
      <div className="flex min-h-[520px] flex-col justify-between bg-[#bfd0c8] p-8 text-black sm:p-12 lg:p-16" data-reveal="right"><div><p className="editorial-eyebrow">Le fonctionnement</p><h2 className="mt-8 max-w-xl text-4xl font-medium leading-[1.02] tracking-[-.045em] sm:text-5xl">Un interlocuteur responsable du résultat d’ensemble.</h2></div><div className="mt-16 grid gap-5 border-t border-black/35 pt-7 sm:grid-cols-2"><p className="text-sm leading-7 text-black/65">Badre garde la stratégie, les arbitrages et la relation de direction. Les spécialistes adaptés sont mobilisés selon la mission.</p><p className="text-sm leading-7 text-black/65">Vous gagnez la cohérence d’un pôle intégré sans financer une structure inutilement lourde.</p></div></div>
    </section>
